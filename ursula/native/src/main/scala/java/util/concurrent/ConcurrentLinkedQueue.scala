...
// https://github.com/scala-native/scala-native/blob/main/javalib/src/main/scala/java/util/concurrent/ConcurrentLinkedQueue.scala
// to avoid some issues with an implementation that is provided in the os-lib ipackage.
// ---
...
class ConcurrentLinkedQueue[E]()
  ...
  private def removeNode(node: Node[E]): Unit = {
    if (node eq head) {
      poll(): Unit // : Unit add to avoid  discarded non-Unit value
      ...
      if (current eq null) {
        null.asInstanceOf[
          E
        ]: Unit // : Unit add to avoid  discarded non-Unit value
      } else {
        _size -= 1
